...
package. We are currently using a previous rebuilt package with a corresponding
...
The current rebuilder system as implemented before this thesis, the was an
...
an ``intoto'' client which can be used as a drop-in replacement of the ``http''
...
communicating with in an configuration file. We will then request each of the
...
do this we need to implement a few thing.
...
This should be validated locally and inherently no need to trust the output of
the rebuilders.
...
\begin{enumerate}
    ...
    \item Fetch consistency proof and validate this
    \item Store new tree root in place of old tree root
    \item Fetch available entries on package
...
merkle tree roots. Thus our additions to the ``intoto'' transport amounts to 5
functions.
...
% TODO: Appendix to apt-transport implementations and commits
...
is used for making sure threshold of available, and good, linkmetadata exists for
...
the merkle tree root present when there was 4 leaf nodes for no particular
